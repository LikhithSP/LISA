...
const translations = {
  English: {
    ...
    educationLevel: "Current Literacy Level",
    selectEducation: "Select starting literacy level",
    ...
    "Level 1: Cannot read lettersOption": "Level 1: Cannot read letters",
    "Level 2: Can read letters but not wordsOption": "Level 2: Can read letters but not words",
    "Level 3: Can read words but not sentencesOption": "Level 3: Can read words but not sentences",
    "Level 4: Can read sentences but struggles with understandingOption": "Level 4: Can read sentences but struggles with understanding",
    "Level 5: Can read and understand basic contentOption": "Level 5: Can read and understand basic content",
  ...
  Hindi: {
    ...
    educationLevel: "वर्तमान साक्षरता स्तर",
    selectEducation: "प्रारंभिक साक्षरता स्तर चुनें",
    ...
    OtherOption: "अन्य",
    // Education Dropdown Options
    "Level 1: Cannot read lettersOption": "स्तर 1: अक्षरों को नहीं पढ़ सकते",
    "Level 2: Can read letters but not wordsOption": "स्तर 2: अक्षर पढ़ सकते हैं लेकिन शब्द नहीं",
    "Level 3: Can read words but not sentencesOption": "स्तर 3: शब्द पढ़ सकते हैं लेकिन वाक्य नहीं",
  ...
  Kannada: {
    ...
    educationLevel: "ಪ್ರಸ್ತುತ ಸಾಕ್ಷರತಾ ಹಂತ",
    selectEducation: "ಆರಂಭಿಕ ಸಾಕ್ಷರತಾ ಹಂತವನ್ನು ಆರಿಸಿ",
    ...
    OtherOption: "ಇತರೆ",
    // Education Dropdown Options
    "Level 1: Cannot read lettersOption": "ಹಂತ 1: ಅಕ್ಷರಗಳನ್ನು ಓದಲು ಸಾಧ್ಯವಿಲ್ಲ",
    "Level 2: Can read letters but not wordsOption": "ಹಂತ 2: ಅಕ್ಷರಗಳನ್ನು ಓದಬಹುದು ಆದರೆ ಪದಗಳಲ್ಲ",
  ...
  Telugu: {
    ...
    educationLevel: "ప్రస్తుత అక్షరాస్యత స్థాయి",
    selectEducation: "ప్రారంభ అక్షరాస్యత స్థాయిని ఎంచుకోండి",
    ...
    OtherOption: "ఇతర",
    // Education Dropdown Options
    "Level 1: Cannot read lettersOption": "స్థాయి 1: అక్షరాలు చదవలేరు",
    "Level 2: Can read letters but not wordsOption": "స్థాయి 2: అక్షరాలు చదవగలరు కానీ పదాలు చదవలేరు",
  ...
  Tamil: {
    ...
    educationLevel: "தற்போதைய எழுத்தறிவு நிலை",
    selectEducation: "ஆரம்ப எழுத்தறிவு நிலையைத் தேர்ந்தெடுக்கவும்",
    ...
    OtherOption: "மற்றவை",
    // Education Dropdown Options
    "Level 1: Cannot read lettersOption": "நிலை 1: எழுத்துக்களைப் படிக்க முடியாது",
...
const assessmentQuestions = {
  ...
  adult_level_3: {
    ...
    },
    {
      ...
      Kannada: "ಪ್ರತಿ ತಿಂಗಳು ಗಳಿಸುವ ಹಣದ (ಸಂಬಳ) ಕಾಗುಣಿತವನ್ನು ಹುಡುಕಿ:",
...
function App() {
  ...
  const getCategory = (age, selectedLevelStr) => {
    const numAge = Number(age) || 30;
    let ageGroup = "adult";
    if (numAge >= 5 && numAge <= 12) ageGroup = "child";
    else if (numAge >= 13 && numAge <= 18) ageGroup = "teen";
    else if (numAge >= 19 && numAge <= 59) ageGroup = "adult";
    else ageGroup = "senior";
    
    let levelNum = 1;
    if (selectedLevelStr && selectedLevelStr.includes("Level 2")) levelNum = 2;
    else if (selectedLevelStr && selectedLevelStr.includes("Level 3")) levelNum = 3;
    else if (selectedLevelStr && selectedLevelStr.includes("Level 4")) levelNum = 4;
    else if (selectedLevelStr && selectedLevelStr.includes("Level 5")) levelNum = 5;
    
    return `${ageGroup}_level_${levelNum}`;
  };
  ...
  const diagnoseLevel = (score, startingLevelStr) => {
    let currentLvl = 1;
    if (startingLevelStr && startingLevelStr.includes("Level 2")) currentLvl = 2;
    else if (startingLevelStr && startingLevelStr.includes("Level 3")) currentLvl = 3;
    else if (startingLevelStr && startingLevelStr.includes("Level 4")) currentLvl = 4;
    else if (startingLevelStr && startingLevelStr.includes("Level 5")) currentLvl = 5;

    if (score >= 8) {
      return Math.min(5, currentLvl + 1);
    } else if (score <= 3) {
      return Math.max(1, currentLvl - 1);
    } else {
      return currentLvl;
    }
  };
  ...
  const handleSubmitAssessment = async (e) => {
    ...
    const diagnosedLevel = diagnoseLevel(score, profile?.education_level);
    const percentage = (score / 10) * 100;
    const lang = selectedLanguage || "English";
    const levelStr = getLocalizedLevelName(diagnosedLevel, lang);
    const summary = `${levelStr} (${getSummary(score, lang)})`;
              ...
              <form onSubmit={handleSubmitAssessment} className="assessment-form">
                ...
                <div className="assessment-submit-bar">
                  <button
                    ...
                    disabled={answeredCount < 10 || submitting}
                  >
          ...
          {assessmentState === "results" && currentAttemptResult && (
            <div className="results-card">
              ...
              <div className="summary-section">
                ...
                <p className="summary-text">{currentAttemptResult.summary}</p>
              </div>
          ...
          {assessmentState === "not_started" && (
            <div className="dashboard-home-card">
              <div className="welcome-banner">
                ...
                {editingProfile ? (
                  <div className="profile-edit-box" style={{ display: "flex", gap: "14px", alignItems: "center", marginTop: "10px", flexWrap: "wrap" }}>
                    <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "0.9rem" }}>
                      Age:
                      <input
                        type="number"
                        style={{ width: "80px", padding: "8px 12px", borderRadius: "10px", border: "1px solid var(--line)" }}
                        value={editAge}
                    ...
                    <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "0.9rem" }}>
                      Education:
                      <select
                        style={{ width: "200px", padding: "8px 12px", borderRadius: "10px", border: "1px solid var(--line)" }}
                        value={editEdLevel}
                        onChange={(e) => setEditEdLevel(e.target.value)}
                      >
                        {literacyLevels.map(level => (
                          <option key={level} value={level}>{t(level + "Option")}</option>
                        ))}
                      </select>
                    </label>
                    <button
              ...
              <div className="empty-state-assessment" style={{ textAlign: "center", padding: "2.5rem 1.5rem", background: "var(--card-bg)", borderRadius: "12px", border: "1px solid var(--border)", maxWidth: "600px", margin: "0 auto" }}>
                <p className="intro-copy" style={{ fontSize: "1.15rem", marginBottom: "1.5rem", lineHeight: "1.6" }}>
                  Welcome to your personalized learning space! Let's start with a quick 10-question initial assessment. This helps us customize future material.
                </p>
                <button
                  type="button"
                ...
                >
                  {profile?.education_level ? t("retakeAssessmentBtn") : t("takeAssessmentBtn")}
                </button>
              </div>
            </div>
          )}
            ...
            <form className="auth-form active" onSubmit={handleRegister}>
              ...
              <label>
                ...
                <select name="educationLevel" required defaultValue="" disabled={submitting}>
                  ...
                  {literacyLevels.map((educationLevel) => (
